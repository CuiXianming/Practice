const vm = new Vue({
  ...
  computed: {
    checkedList() {
      return this.courseList.filter((element) => element.checked);
    },

    selectedTotalPrice() {
      let price = 0;
      this.checkedList.forEach(
        (element) => (price += element.price * element.cart),
      );
      return price;
    },
    ...
    checkedAll: {
      get() {
        return this.courseList.every((element) => element.checked);
      },
      set(val) {
        this.courseList.forEach((element) => (element.checked = val));
      },
    },
  },
});

vm.courseList.forEach((element) => {
